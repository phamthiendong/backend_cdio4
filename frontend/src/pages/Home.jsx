...
import {
  Search, Stethoscope, Hospital, Users, BookOpen, Calendar,
...
function SectionHeader({ icon, title, action, onAction }) {
  return (
    <div className="flex items-center justify-between mb-4">
      ...
      {action && (
        <button onClick={onAction} className="text-sm md:text-base inline-flex items-center gap-1 text-blue-600 hover:underline">
          {action}
    ...
    <div className="rounded-2xl p-4 bg-white shadow-sm hover:shadow-md border border-gray-100">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <button onClick={()=>navigate(`/booking/${d.id}`)} className="mt-3 inline-flex items-center px-3 py-1.5 rounded-full bg-blue-600 text-white text-sm hover:bg-blue-700">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {c.tags.map((t, i) => (
              <span key={i} className="px-2 py-1 bg-blue-50 text-blue-700 rounded-full text-xs">{t}</span>
            ))}
          </div>
          <div className="flex items-center gap-2 text-sm mt-2">
      ...
      <div className="px-5 md:px-10 py-10 md:py-16 flex flex-col md:flex-row items-center gap-8">
        <div className="flex-1">
          ...
          <div className="mt-6 bg-white rounded-2xl p-2 flex items-center gap-2 w-full max-w-xl">
            ...
            />
            <button onClick={() => navigate(`/search?q=${encodeURIComponent(q)}`)} className="px-4 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700">
              Tìm kiếm
            </button>
          </div>
  ...
  return (
    <>
      ...
      <section id="specialties" className="mt-10 md:mt-14">
        <SectionHeader icon={<Stethoscope className="w-6 h-6 text-blue-600" />} title="Khám Chuyên khoa" action="Xem tất cả" onAction={()=>navigate("/specialties")} />
      ...
      <section id="doctors" className="mt-12 md:mt-16">
        <SectionHeader icon={<Users className="w-6 h-6 text-blue-600" />} title="Bác sĩ nổi bật" action="Xem tất cả" onAction={()=>navigate("/doctors")} />
      ...
      <section id="clinics" className="mt-12 md:mt-16">
        <SectionHeader icon={<Hospital className="w-6 h-6 text-blue-600" />} title="Cơ sở y tế uy tín" action="Xem tất cả" onAction={()=>navigate("/clinics")} />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
      ...
      <section id="handbooks" className="mt-12 md:mt-16">
        <SectionHeader icon={<BookOpen className="w-6 h-6 text-blue-600" />} title="Cẩm nang sức khoẻ" action="Xem thêm" onAction={()=>navigate("/knowledge")} />
      ...
      <section className="mt-12 md:mt-16">
        <div className="rounded-3xl p-6 md:p-10 bg-blue-600 text-white flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
          ...
          <button onClick={()=>navigate("/search")} className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-white text-blue-700 font-medium hover:bg-blue-50">
